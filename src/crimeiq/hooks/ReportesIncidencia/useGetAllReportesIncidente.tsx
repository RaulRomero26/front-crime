...
export const useGetAllReportesIncidente = ({perPageReq}:GetAllReportesTareasProps) => {

    const [page, setPage] = useState(1);
    const [perPage,_setPerPage] = useState(perPageReq)

    const { isLoading, isError, error, data: reportes = [], isFetching} = useQuery({
        queryKey: ['incidente',{page,perPage}],
        queryFn: () => crimeiqActions.useGetAllReportesIncidente({page,perPage}),
