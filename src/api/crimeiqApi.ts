...
const { VITE_BACKEND_SERVER_CRIMEIQ } = getEnvVariables()
//Se crea y exporta la funcion que genera nuestro intermediario entre el frontend
//y el backend
export const crimeiqApi = axios.create({
    baseURL: VITE_BACKEND_SERVER_CRIMEIQ
});
...
// crimeiqApi.interceptors.request.use((config: InternalAxiosRequestConfig) => {
...
// con status 400, manejadas por el servidor sacar al usuario no autenticado de la aplicación
  crimeiqApi.interceptors.response.use(
      response => response,
      error => {
